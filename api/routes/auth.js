...
// POST /api/auth/forgot-password - NEW ENDPOINT FOR EMAIL TOKEN
router.post(
  '/forgot-password',
  [
    body('email').isEmail().normalizeEmail().withMessage('Valid email is required')
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        ...
        return res.json({ 
          success: true, 
          message: 'If an account exists, check your inbox/spam for reset link.' 
        });
      }
      ...
      // Nodemailer setup (Gmail/GMass SMTP)
...
<p>Click to reset: <a href="${resetUrl}">Reset Link</a></p>
<p>Expires in 1 hour.</p>
      ...
      res.json({ 
        success: true, 
        message: 'Reset link sent! Check inbox/spam folder.' 
      });
